...
func (e *ExposeInfo) UnmarshalJSON(data []byte) error {
	type Tmp ExposeInfo
	var tmp Tmp
...
func (a ExposeAccess) String() string {
	var parts []string
	if a.Pub {
...
func (a *ExposeAccess) UnmarshalJSON(data []byte) error {
	var tmp uint
	if err := json.Unmarshal(data, &tmp); err != nil {
		return err
	}
