...
export type UserData = {
	...
	fullname: string;
	role: string;
	// True while the user is still on a password an admin chose for them.
	// The app layout blocks every page behind a password change until the
	// server clears it, so this must default to false rather than
	// undefined - a missing claim must not read as "reset required".
...
function setAccessToken(token: string) {
	...
	if (token !== '') {
		...
		userDataValue.set({
			...
			reset_password: claims.reset_password ?? false
		});
	} else {
		userDataValue.set({
			exp: 0,
