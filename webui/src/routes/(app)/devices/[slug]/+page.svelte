...
<main class="">
	{#if dev}
		...
		<div class="flex flex-col gap-2">
			...
			<div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 items-center">
				<div>Device Name</div>
				<div class="font-mono bg-muted p-1 rounded-md">{dev.name}</div>
				<div>Device ID</div>
				<div class="font-mono bg-muted p-1 rounded-md">{dev.id}</div>
				<div>Client ID</div>
				<div class="font-mono bg-muted p-1 rounded-md">{dev.clientID || 'Unknown'}</div>
				<div>Online</div>
				<div class="font-mono bg-muted p-1 rounded-md">{dev.online}</div>
				{#if dev.lastSeen}
					<div>Last Seen</div>
					<div class="font-mono bg-muted p-1 rounded-md">{toHumanDate(attributeToDate(dev.lastSeen))}</div>
				{/if}
				{#if dev.batteryLevel}
					<div>Battery</div>
					<div class="font-mono bg-muted p-1 rounded-md">{Number(dev.batteryLevel)}%</div>
				{/if}
			...
			<div class="flex flex-col md:flex-row gap-2 overflow-x-scroll">
				{#each dev.services as srv, i (srv.id)}
					<ServiceEnumerator showDeviceName={false} {deviceName} deviceID={dev.id} online={dev.online} service={srv} />
				{/each}
			</div>
	...
	{:else}
		<div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 items-center">
			...
			<div>Server:</div>
			<div class="font-mono bg-muted p-1 rounded-md">
